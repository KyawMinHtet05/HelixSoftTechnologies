...
class ClinicalDataProcessor:
    ...
    def get_file_list(self, status_queue=None):
        """Get list of ALL files from server (not just CSV)"""
        ...
        try:
            ...
            if status_queue and files:
                status_queue.put((f"✅ Found {len(files)} files", "success"))
            elif status_queue:
                status_queue.put(("No files found", "warning"))
...
class ClinicalDataValidator:
    ...
    def validate_selected_files(self, ftp, files, status_queue):
        ...
        for filename in files:
            if not filename.upper().endswith('.CSV'):
                status_queue.put((f"\n⚠️ Skipping non-CSV file: {filename}", "warning"))
                continue
            ...
            try:
                ...
                if self._validate_filename_pattern(filename, status_queue):
                    ...
                    status_queue.put((f"❌ INVALID: {filename} (wrong filename pattern)", "error"))
                    invalid_count += 1
    ...
    def process_selected_files(self, ftp, files, status_queue):
        ...
        for filename in files:
            ...
            status_queue.put((f"\n{'='*60}", "info"))
            status_queue.put((f"⚙️ Processing: {filename}", "info"))
...
class ClinicalDataGUI:
    ...
    def __init__(self, root):
        ...
        home_dir = Path.home()
        self.download_dir = tk.StringVar(value=str(home_dir / "ClinicalData" / "Downloads"))
        self.archive_dir = tk.StringVar(value=str(home_dir / "ClinicalData" / "Archive"))
        self.error_dir = tk.StringVar(value=str(home_dir / "ClinicalData" / "Errors"))
    ...
    def create_widgets(self):
        ...
        main_frame.grid_columnconfigure(0, weight=1)  # Column 1: FTP & Directories
        main_frame.grid_columnconfigure(1, weight=2)  # Column 2: Server Files
        main_frame.grid_columnconfigure(2, weight=2)  # Column 3: Log/Processing
        
        main_frame.grid_rowconfigure(0, weight=1)
        
        # ===== COLUMN 1: FTP Connection & Local Directories =====
        left_column = ttk.Frame(main_frame)
        left_column.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), padx=(0, 10))
        left_column.grid_columnconfigure(0, weight=1)
        left_column.grid_rowconfigure(0, weight=1)  # Frames will fill vertically
        
        # FTP Connection Frame
        ftp_frame = ttk.LabelFrame(left_column, text="FTP Connection", padding="15")
        ftp_frame.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N), pady=(0, 10))
        ...
        ttk.Entry(ftp_frame, textvariable=self.ftp_user, width=25).grid(row=1, column=1, sticky=tk.W, pady=5)
        
        ttk.Label(ftp_frame, text="Password:").grid(row=2, column=0, sticky=tk.W, padx=(0, 10))
        ttk.Entry(ftp_frame, textvariable=self.ftp_pass, show="*", width=25).grid(row=2, column=1, sticky=tk.W, pady=5)
        
        # Connect/Disconnect buttons
        btn_frame_ftp = ttk.Frame(ftp_frame)
        btn_frame_ftp.grid(row=3, column=0, columnspan=2, pady=(10, 0))
        
        self.connect_btn = ttk.Button(btn_frame_ftp, text="🔌 Connect", command=self.connect_to_server, width=12)
        self.connect_btn.pack(side=tk.LEFT, padx=2)
        
        self.disconnect_btn = ttk.Button(btn_frame_ftp, text="❌ Disconnect", command=self.disconnect_from_server, 
                                        width=15, state=tk.DISABLED)
        self.disconnect_btn.pack(side=tk.LEFT, padx=2)
        
        # Status label
        self.status_label = ttk.Label(ftp_frame, text="🔴 Disconnected", foreground="red")
        self.status_label.grid(row=4, column=0, columnspan=2, sticky=tk.W, pady=(10, 0))
        
        # Local Directories Frame
        dir_frame = ttk.LabelFrame(left_column, text="Local Directories", padding="15")
        dir_frame.grid(row=1, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
        ...
        for i, (label, var) in enumerate(directories):
            ...
            entry = ttk.Entry(dir_frame, textvariable=var, width=25, state='readonly')
            entry.grid(row=i, column=1, sticky=tk.EW, pady=5, padx=(0, 5))
            ttk.Button(dir_frame, text="Browse...", command=lambda v=var: self.browse_directory(v)).grid(row=i, column=2)
        
        # Additional action buttons in directory frame
        action_frame = ttk.Frame(dir_frame)
        action_frame.grid(row=4, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(15, 0))
        action_frame.grid_columnconfigure(0, weight=1)
        action_frame.grid_columnconfigure(1, weight=1)
        
        ttk.Button(action_frame, text="📂 Open Error Log", command=self.open_error_log).grid(row=0, column=0, sticky=tk.EW, padx=(0, 5))
        ttk.Button(action_frame, text="🗑️ Clear Log", command=self.clear_log).grid(row=0, column=1, sticky=tk.EW)
        
        # ===== COLUMN 2: Server Files =====
        middle_column = ttk.Frame(main_frame)
        middle_column.grid(row=0, column=1, sticky=(tk.W, tk.E, tk.N, tk.S), padx=(0, 10))
        middle_column.grid_columnconfigure(0, weight=1)
        middle_column.grid_rowconfigure(0, weight=1)
        
        server_files_frame = ttk.LabelFrame(middle_column, text="Server Files", padding="15")
        server_files_frame.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        server_files_frame.grid_columnconfigure(0, weight=1)
        server_files_frame.grid_rowconfigure(1, weight=1)  # Listbox gets weight
        
        # Search controls
        search_frame = ttk.Frame(server_files_frame)
        ...
        ttk.Button(search_frame, text="🔍 Search", command=self.filter_file_list, width=10).grid(row=0, column=2, padx=(0, 5))
        ttk.Button(search_frame, text="🔄 Refresh", command=self.refresh_file_list, width=10).grid(row=0, column=3)
        
        # File listbox
        list_container = ttk.Frame(server_files_frame)
        list_container.grid(row=1, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
        ...
        # File count label
        self.file_count_label = ttk.Label(server_files_frame, text="No files loaded")
        self.file_count_label.grid(row=2, column=0, sticky=tk.W, pady=(0, 10))
        
        # Action buttons
        action_frame = ttk.Frame(server_files_frame)
        action_frame.grid(row=3, column=0, sticky=(tk.W, tk.E))
        action_frame.grid_columnconfigure(0, weight=1)
        action_frame.grid_columnconfigure(1, weight=1)
        
        self.validate_btn = ttk.Button(action_frame, text="🔍 Validate Selected File", 
                                      command=self.validate_selected, state=tk.DISABLED)
        self.validate_btn.grid(row=0, column=0, sticky=tk.EW, padx=(0, 5))
        
        self.process_btn = ttk.Button(action_frame, text="🚀 Process Selected File", 
                                     command=self.process_selected, state=tk.DISABLED)
        self.process_btn.grid(row=0, column=1, sticky=tk.EW)
        
        # ===== COLUMN 3: Processing Log =====
        right_column = ttk.Frame(main_frame)
        right_column.grid(row=0, column=2, sticky=(tk.W, tk.E, tk.N, tk.S))
        right_column.grid_columnconfigure(0, weight=1)
        right_column.grid_rowconfigure(0, weight=1)
        
        log_frame = ttk.LabelFrame(right_column, text="Processing Log", padding="15")
        log_frame.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        log_frame.grid_columnconfigure(0, weight=1)
        log_frame.grid_rowconfigure(0, weight=1)  # Log text gets weight
    ...
    def on_file_selection_change(self, event):
        ...
        if selection and self.processor and self.processor.connected:
            ...
            self.file_count_label.config(text=f"Selected: {selected_file}")
        else:
            self.validate_btn.config(state=tk.DISABLED)
            self.process_btn.config(state=tk.DISABLED)
    ...
    def update_file_listbox(self):
        ...
        csv_count = len([f for f in self.displayed_files if f.upper().endswith('.CSV')])
        total_count = len(self.displayed_files)
        self.file_count_label.config(text=f"📁 {total_count} files ({csv_count} CSV files)")
        
        self.log_message(f"📁 Loaded {total_count} files from server", "info")
    ...
    def filter_file_list(self, event=None):
        ...
        if search_term and not self.displayed_files:
            ...
            self.log_message(f"🔍 Filtered: showing {total_count} files matching '{search_term}'", "info")
